...
export default function ColorMatchPanel({ hsb, playerHex, started, locked, memorizing, usedHint, progress, onChange, onStart, onSubmit, onHint }: Props) {
  return (
    <div className="relative min-h-[360px] overflow-hidden rounded-[2rem] shadow-[0_22px_60px_rgba(79,70,229,0.16)]" style={{ backgroundColor: playerHex }}>
      ...
      <div className="absolute bottom-5 left-5 top-16 z-10 flex w-[116px] items-center justify-center rounded-[1.7rem] bg-white/18 px-4 py-5 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.24)] backdrop-blur-md">
      ...
      <div className="absolute bottom-5 right-5 z-10 flex items-center gap-3">
        <button
          ...
          onClick={onHint}
          aria-label="Hint"
          className="grid h-13 w-13 place-items-center rounded-full bg-[#e5f2ea] text-xl font-black text-slate-800 shadow-[0_10px_24px_rgba(15,23,42,0.18)] transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40"
        >
          ?
        </button>
        {!started ? (
          <button
            type="button"
            onClick={onStart}
            className="grid h-13 min-w-24 place-items-center rounded-full bg-white px-5 text-base font-black text-slate-900 shadow-[0_10px_24px_rgba(15,23,42,0.22)] transition hover:scale-105"
          >
            Start
          </button>
        ) : (
          <button
            ...
            className="grid h-13 w-13 place-items-center rounded-full bg-white text-2xl font-black text-slate-900 shadow-[0_10px_24px_rgba(15,23,42,0.22)] transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40"
          >
            ✓
          </button>
        )}
      ...
      <div className="absolute bottom-5 left-[150px] z-10 rounded-full bg-black/16 px-3 py-1 font-mono text-xs font-black uppercase tracking-wide text-white shadow-sm backdrop-blur-md">
        {playerHex}
      </div>
    </div>
  );
...
function VerticalSlider({ label, value, min, max, disabled, gradient, onChange }: { label: string; value: number; min: number; max: number; disabled: boolean; gradient: string; onChange: (value: number) => void }) {
  ...
  return (
    <label className="relative h-[245px] w-5">
      ...
      />
      <span className="pointer-events-none absolute left-1/2 z-20 h-5 w-5 rounded-full border-[3px] border-white bg-white shadow-[0_4px_12px_rgba(15,23,42,0.28)]" style={{ top: `${100 - percent}%`, transform: "translate(-50%, -50%)" }} />
    </label>
  );
}
