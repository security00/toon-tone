...
function SwatchStripe({ color, label }: { color: string; label: string }) {
  return (
    <div className="flex max-w-full items-center gap-2 rounded-full bg-white/80 px-3 py-2 text-xs font-black text-slate-700 shadow-sm ring-1 ring-slate-200 backdrop-blur">
...
export default function CharacterMemoryCard({ question, reveal, playerHex, locked }: Props) {
  ...
  return (
    <div className="h-full min-w-0 max-w-full overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 sm:rounded-[2rem] sm:shadow-[0_22px_60px_rgba(79,70,229,0.14)]">
      ...
      <div className="relative min-h-[220px] w-full overflow-hidden transition-colors duration-200 sm:aspect-square sm:min-h-[360px]" style={{ backgroundColor: displayHex }}>
        ...
        <div className="relative flex min-h-[220px] w-full flex-col justify-between p-4 sm:h-full sm:min-h-0 sm:p-6">
          <div className="flex justify-end">
            <SwatchStripe color={displayHex} label={showTarget ? question.targetColorHex : "Current"} />
          </div>
